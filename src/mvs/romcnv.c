...
// GEKKO
static UINT8* NeoTileAttrib[32] = { NULL, };
...
int NeoInitSprites()
{
    ...
    NeoTileAttrib[0] = (UINT8*)malloc( (memory_length_gfx3 >> 7) + 1);

    for (int i = 0; i < gfx_total_elements[TILE_SPR]; i++)
   {
        bool bTransparent = true;
        for (INT32 j = i << 7; j < (i + 1) << 7; j++)
      {
            if (memory_region_gfx3[j])
         {
                bTransparent = false;
                break;
            }
   ...
   for (INT32 i = 0; i < 8; i++) {
      ...
      for (UINT8* pTile = pStart; pTile < pEnd; pTile += 128) {
         ...
         for (INT32 n = 0; n < 32; n++) {
                        ((UINT32*)pTile)[n] = changed_endian(data[n]);
         }
      }
   }
}
...
static void neogeo_decode_spr(UINT8 *mem, UINT32 length, UINT8 *usage)
{
    ...
    {
        gfxdata = (unsigned int *)&mem[128 * i];
...
// wiimpathy : Force sound file creation
    ...
    //{
        if (load_rom_sound1())
        {
            if (encrypt_snd1)
            {
                ...
                case INIT_kf2k3bl:    neo_pcm2_swap(5);        break;
                case INIT_kf2k3pl:    neo_pcm2_swap(5);        break;
    ...
    if (encrypt_gfx2 || encrypt_gfx3)
    {
        if (load_rom_gfx2() && load_rom_gfx3())
        {
            ...
            // GEKKO
            //neogeo_decode_spr(memory_region_gfx3, memory_length_gfx3, gfx_pen_usage[TILE_SPR]); // GXGEO
            NeoDecodeSprites(memory_region_gfx3, memory_length_gfx3); // FBA
        }
        else
        {
            return 0;
...
//    sprintf(version, "MVS_V%d%d\0", VERSION_MAJOR, VERSION_MINOR);
    ...
    fwrite(NeoTileAttrib[0], (memory_length_gfx3 >> 7) + 1, 1, fp);
    fclose(fp);
    ...
    {
// GEKKO : Force srom text file creation
        load_rom_gfx2();

        if ((fp = fopen("srom", "wb")) == NULL) goto error;
    }

        int size = changed_endian(memory_length_gfx2);
        fwrite(&size, sizeof(int), 1, fp);
        fclose(fp);

        if ((fp = fopen("srom", "ab")) == NULL) goto error;

        NeoDecodeText(0, memory_length_gfx2, memory_region_gfx2, memory_region_gfx2);
        fwrite(memory_region_gfx2, 1, memory_length_gfx2, fp);
        fclose(fp);

// GEKKO : Force sound file creation
...
int main(int argc, char *argv[])
{
    ...
    printf(" ROM converter for fbalpha2012 neogeo \n");
    printf("----------------------------------------------\n\n");
#endif
    ...
    if (all)
    {
        ...
        printf("Please copy these files to directory \"/retroarch/system/\".\n");
#endif
    }
    else
    {
        ...
        *p = '\0';
...
#endif
        {
            ...
            printf("Please copy \"cache%c%s_cache\" folder to directory \"/retroarch/system/\".\n", delimiter, game_name);
#endif
        }
        free_memory();
    }
